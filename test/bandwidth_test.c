...
server_microtcp (uint16_t listen_port, const char *file)
{
    ...
    accepted = microtcp_accept(&sock, (struct sockaddr *) &client_addr, sizeof (client_addr));
        ...
        switch (opt)
        {
            ...
            case 'f':
                ...
                if(access(filestr, F_OK) != 0){
                    perror("file doen't exitst");
                    exit(EXIT_FAILURE);
                }
                // Get the absolute path
                //replace it in the rest of the file !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
                if (realpath(filestr, absolutePath) == NULL) {
